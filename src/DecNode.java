...
public class DecNode implements Node {
    ...
    public ArrayList<SemanticError> checkSemantics(SymbolTable ST, int _nesting) {
        ...
        if (type instanceof Type) {
            ...
            Type declaredType = (Type) type; //assegno a declaredType il tipo che è stato dichiarato nella grammatica (verifico che il tipo sia presente nella grammatica con instanceof)
            Type actualType = ST.getType(id, nesting); //ricavo il tipo e il livello di annidamento di id
    ...
    public String codeGeneration() {
        StringBuilder sb = new StringBuilder(); //costruisce il codice generato
        // Generate code for the body statements
        for (Node statement : body) {
            sb.append(statement.codeGeneration()); //genera il codice corrispondente all'istruzione presa in esame
        }
        return sb.toString();
    }
